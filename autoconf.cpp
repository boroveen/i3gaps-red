...
printf("#                  #\n");
printf("####################\n");
system("sudo apt install i3-wm  compton i3lock feh i3blocks i3status");
system("sudo apt install xinit arandr");
system("sudo add-apt-repository ppa:elementary-os/daily");
system("sudo apt-get update");
system("sudo apt-get install pantheon-files");
printf("####################\n");
printf("#                  #\n");
printf("#                  #\n");
printf("#                  #\n");
...
while(!gg){
Color();
if(y1==1) attron(COLOR_PAIR(3));
else attron(COLOR_PAIR(2));
if(!i3wm) attron(COLOR_PAIR(3));
mvprintw(y/2,x/2,"install i3wm");
if(y1==2) attron(COLOR_PAIR(3));
else attron(COLOR_PAIR(2));
if(!i3gaps) attron(COLOR_PAIR(3));
mvprintw(y/2+1,x/2,"install i3gaps");
if(y1==3) attron(COLOR_PAIR(3));
else attron(COLOR_PAIR(2));
mvprintw(y/2+2,x/2,"install");
if(y1==4) attron(COLOR_PAIR(3));
else attron(COLOR_PAIR(2));
